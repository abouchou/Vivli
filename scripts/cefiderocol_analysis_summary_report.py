...
def create_model_performance_plots(results, X_test, y_test):
    ...
    for i, (name, result) in enumerate(results.items()):
        ...
        if i >= 2:  # Only show first 2 confusion matrices to avoid subplot overflow
            break
...
def convert_to_pdf():
    ...
    try:
        ...
        html_doc.write_pdf('outputs/cefiderocol_comprehensive_analysis_report.pdf', 
                          font_config=font_config)
        
        print("PDF report saved as 'outputs/cefiderocol_comprehensive_analysis_report.pdf'")
        
    except ImportError:
        print("Warning: Required packages (markdown, weasyprint) not available for PDF conversion.")
        print("Please install: pip install markdown weasyprint")
...
def main():
    ...
    print("✅ Comprehensive report generated in markdown and PDF formats")
