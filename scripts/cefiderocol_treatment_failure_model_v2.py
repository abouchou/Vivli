...
def load_and_prepare_data():
    ...
    # Approche plus réaliste : utiliser seulement les résistances aux autres antibiotiques
    # pour prédire le risque d'échec du céfidérocol
    ...
    # Cible : risque élevé d'échec (basé sur les résistances aux autres antibiotiques)
    # Ne pas utiliser directement la résistance au céfidérocol pour éviter la fuite de données
    sidero_data["treatment_failure_risk"] = (
        (sidero_data["other_resistance_count"] >= 2) |
        (sidero_data["meropenem_mic_resistant"] == 1)  # Résistance au méropénème comme indicateur
    ).astype(int)
...
def prepare_features(sidero_data):
    ...
    sidero_data["resistance_pattern_score"] = (
        sidero_data["meropenem_mic_resistant"] * 2 + 
        sidero_data["ciprofloxacin_mic_resistant"] + 
        sidero_data["colistin_mic_resistant"]
    )
    
    numeric_features.extend(["meropenem_ciprofloxacin_ratio", "meropenem_colistin_ratio", "resistance_pattern_score"])
...
def analyze_performance(results, X_test, y_test):
    ...
    plt.tight_layout()
    plt.savefig('outputs/plots/cefiderocol_model_performance_v2.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def analyze_feature_importance(results, X_train, feature_names):
    ...
    plt.tight_layout()
    plt.savefig('outputs/plots/cefiderocol_feature_importance_v2.png', dpi=300, bbox_inches='tight')
    plt.show()
...
Le modèle développé présente des performances {results[best_model_name]['auc']:.3f}, ce qui reflète la complexité de la prédiction de l'échec du traitement au céfidérocol. Les performances limitées sont justifiées par :
...
"""
    ...
    with open('outputs/cefiderocol_treatment_failure_report_v2.md', 'w', encoding='utf-8') as f:
        f.write(report)
    
    print("Rapport sauvegardé dans 'outputs/cefiderocol_treatment_failure_report_v2.md'")
...
def main():
    ...
    print("Le rapport complet a été sauvegardé dans 'outputs/cefiderocol_treatment_failure_report_v2.md'")
